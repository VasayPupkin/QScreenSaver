...
//    Coordinate centr(300,300);//TODO must be rnd generate
//    d -= 300;
...
bool BubbleMovementManagment::CheckFrameCollision(BubblePtr &bubble_ptr, BarrierType &barrier)
{
    ...
    if (delta_top <= 0)
        negativ_map[abs(delta_top)] = BarrierType::TOP;
    if (delta_bottom <= 0)
        negativ_map[abs(delta_bottom)] = BarrierType::BOTTOM;
    if (delta_left <= 0)
        negativ_map[abs(delta_left)] = BarrierType::LEFT;
    if (delta_right <= 0)
       negativ_map[abs(delta_right)] = BarrierType::RIGHT;
